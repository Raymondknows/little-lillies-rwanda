...
export default function ResultPinsPage() {
  ...
  const loadPins = async (searchValue = pinSearch) => {
    try {
      setLoadingPins(true);
      const response = await fetch(`${backendUrl}/api/result-pins/pins?search=${encodeURIComponent(searchValue)}&limit=75`, {
        credentials: "include",
      });
                ...
                <tbody className="divide-y divide-border bg-surface/60">
                  {filteredPins.map((pin) => (
                    <tr key={pin.id} className="align-top">
                      ...
                      <td className="px-3 py-3">
                        <span className="rounded-full border border-border bg-background px-2.5 py-1 text-xs font-medium text-foreground">{pin.type || '—'}</span>
                      </td>
                      <td className="px-3 py-3">
                        <span className="rounded-full border border-border bg-background px-2.5 py-1 text-xs font-medium text-foreground">{pin.status || 'ACTIVE'}</span>
                      </td>
                      ...
                      <td className="px-3 py-3">
                        <div className="flex flex-wrap gap-2">
                          <button type="button" onClick={() => handleViewPin(pin)} className="inline-flex items-center gap-1 rounded border border-border bg-background px-2 py-1 text-xs font-medium text-foreground hover:bg-muted/30">
                            <Eye className="h-3.5 w-3.5" /> View
                          </button>
                          <button type="button" onClick={() => handlePrintPin(pin)} className="inline-flex items-center gap-1 rounded border border-border bg-background px-2 py-1 text-xs font-medium text-foreground hover:bg-muted/30">
                            <Printer className="h-3.5 w-3.5" /> Print
                          </button>
                          <button type="button" className="inline-flex items-center gap-1 rounded border border-border bg-background px-2 py-1 text-xs font-medium text-foreground hover:bg-muted/30">
