      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="space-y-6">
          <div className="rounded-3xl border border-[#0A66C2]/20 bg-[#0A66C2]/10 p-6 shadow-sm">
            ...
            <div className="mt-6 rounded-3xl border border-[#0A66C2]/20 bg-white p-4 text-sm text-slate-700">
              <p className="font-semibold text-slate-900">Admissions status</p>
              <p className="mt-1">{admissionStatusLabel}</p>
            </div>

            {(settings?.openingDate || settings?.closingDate) ? (
              <div className="mt-6 rounded-3xl border border-[#0A66C2]/20 bg-white p-4 text-sm text-slate-700">
                <p className="font-semibold text-slate-900">Admissions window</p>
                <p className="mt-1">
                  {settings?.openingDate ? new Date(settings.openingDate).toLocaleDateString() : "Open now"} – {settings?.closingDate ? new Date(settings.closingDate).toLocaleDateString() : "No deadline"}
                </p>
              </div>
            ) : null}
          </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
          ...
          {!settings?.enabled ? (
            <div className="rounded-3xl border border-rose-200 bg-rose-50 p-6 text-sm text-rose-700">
              <p className="font-semibold text-rose-900">Online admissions are disabled.</p>
              <p className="mt-2">This school has temporarily turned off public admission requests. Please contact the school directly using the details on this page.</p>
            </div>
          ) : openingDate && today < openingDate ? (
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-700">
              <p className="font-semibold text-slate-900">Admissions open soon</p>
              <p className="mt-2">Applications will be accepted starting {openingDate.toLocaleDateString()}.</p>
            </div>
          ) : closingDate && today > closingDate ? (
            <div className="rounded-3xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-700">
              <p className="font-semibold text-slate-900">Admissions are closed</p>
              <p className="mt-2">The online admissions window ended on {closingDate.toLocaleDateString()}.</p>
            </div>
          ) : (
