...
export async function GET(request: Request) {
  try {
    ...
    let backendUrl = getBackendUrl(apiBase, '/api/admin/fees/data', search);
    ...
    if (!params.get('schoolId')) {
      try {
        ...
        console.warn('Could not determine current school id for fees data proxy');
      }
    }
    ...
    console.error('Proxy /api/admin/fees/data error:', err);
