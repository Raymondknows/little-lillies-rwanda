...
const forwardRequest = async (request: NextRequest, pathSegments: string[]) => {
  ...
  // If request has no session cookie and no schoolId, inject DEV_SCHOOL_ID for local dev convenience
  try {
    ...
    if (!hasCookie && !params.has('schoolId')) {
      ...
      console.debug('[WHATSAPP-BAILEYS PROXY] Injected DEV_SCHOOL_ID into forwarded request');
    }
  } catch (e) {
    // ignore
  }
  ...
  try {
    ...
    console.error('[WHATSAPP-BAILEYS PROXY] Failed to forward request to backend:', url.toString(), error);
